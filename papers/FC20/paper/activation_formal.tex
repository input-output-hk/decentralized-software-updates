...
a party $P_i$ receives the activation command and let $\activep$ be the set of parties that received this command command. Without loss of generality, let $P_1$ be the first party to receive the update command (note that $t_{P_1}\leq t_{P_i}$ for all $P_i\in\activep$).
...
than $\tcorrupt_2$) for some $t\geq t_{P_1}+\Delta_1$, then
...
The parameter $\Delta_1$ defines how much has to pass until the number of honest parties that have received $(\activate,\cdot)$
is such that the assumption $\asmp_2$ holds. $\Delta_2$ instead represents the time required for the update process to be completed.
