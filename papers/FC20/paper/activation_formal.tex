...
asset \nnote{stake is attributed to some public key (the stakeholder) and not to a particular coin. Unless, by "asset" you mean some cryptocurrency of a PoS blockchain. You explain this in the next paragraph}, on this ledger or elsewhere) under the control of the adversary; that a
...
in order to be prepared to run $\ledger_2$ (and maybe stop running $\ledger_1$) \nnote{So, when we receive this command, we start the preparation NOT the activation of changes?}. The receipt of the input $(\activate,\cdot)$ from the environment 
...
and that the generation of this block trigger the activation protocol.} \nnote{This contradicts with the "preparation" statement above. Is the moment that we receive the update command when we start the preparation, or the moment that the changes take effect i.e., the activation? This must be clarified.}

\nnote{Of course, these are two separate moments. Shouldn't we have two separate commands from the env. for each one? E.g., (upgrade, L2)  and (activate, L2)}
