...
\nknote{"at time $t_{p_i}$ with $p_i\in\activep$". Should this be "at time $t_{P_i}$ with $P_i\in\activep$"?}
...
\nknote{Similarly to my previous comment. Should we say: "$  P_i^{\ledger_1}[t_{P_1 + \Delta_1}] \preceq \ledger^\star$ for some $P_i\in\activep$", since $t_{P_1} + \Delta_1$ is the timepoint where $L_2$ kicks off?}
