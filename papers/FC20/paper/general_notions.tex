...
\item[Completeness:] For every pair $(s,v) \from \sgen(1^\lambda)$, \nnote{For the generator algorithm, shouldn't we use "Gen" as above, instead of "Kgen"? The same comment applies also for all appearances of "Kgen" in the following text}
    and every $m \in \zo{\lambda}$, we have that $\Pr[\Ver(v, m,\Sign(s, m))=0]<\nu(\lambda).$
\item[Consistency (non-repudation):]\nnote{I think it is "non-repudiation"} For any $m$, the probability that $\sgen(1^\lambda)$ generates $(s,v)$
...
where $Q$ denotes the set of the message \nnote{messages} requested by $\adv$ to the oracle $\Sign(s,\cdot)$.
\end{description}
 \end{definition}
