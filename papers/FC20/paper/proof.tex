...
let $t_j$ be the smallest time slot in which $B_j$ appears in \nnote{$\state_2^{P_i}[t_j]$}  $\state_2^{P_i}$ for each $P_i\in\parties$ and let $t_{j+k}$ be smallest time slot in which $B_{j+k}$ appears in \nnote{$\state_2^{P_i}[t_{j+k}]$}
...
Let $t'$ be the smallest time slot in which $B_{j+k}$ appears in $\state_2^{P_i}[t']$ \nnote{Maybe you mean $\check \state_1^{P_i}[t']$ because that is what we have said above: "the parties need to wait that the $(j+k)$-th block of $\state_1$ is part of $\check \state_1^{P_i}[t]$" } for each $P_i\in\parties$.
...
that the blocks $B_{j}, B_{j+1},\dots, B_{j+k}$ \nnote{Maybe you want to say: $B_{j+k}, B_{j+k+1},\dots, B_{j+k+k}$, because $\ledger_2$ starts its life from block $B_{j+k}$ and onwards until $k$ blocks are generated} are generated using $\ledger_1$, but this does not represent an issue since we are assuming that
