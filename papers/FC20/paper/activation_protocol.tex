...
We also assume that the parties in $\activep$ know an index $j$ such that when the $j$-th block is added to the \nnote{stable part of} state of $\ledger_1$ then $\asmp_2$ holds. 
More formally, at time $t$ the $j$-th block is part of \nnote{$\state_1^{P_i}[t]$} $\ledger_1^{P_i}$ for all $P_i\in\activep$.
