...
With respect to the two said attacks, we define next, the notion of a secure 
...
\begin{definition}
	...
	\begin{itemize}
		...
		%a software update that has achieved an honest stake majority approval 
		%of \emph{strength} $\mu$ (i.e., $\frac{H}{2} + \mu$ positive votes 
		%from 
		%the honest parties, where $0 <\mu \leq \frac{H}{2}$), will be allowed 
		%to pass to the next phase in the SU lifecycle, i.e., cannot be blocked 
		%by the adversary stake.
	\end{itemize}
