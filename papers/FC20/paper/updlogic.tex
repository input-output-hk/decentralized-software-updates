...
, we present an indicative example of an update policy that is driven by the software update context. In this example, an update policy is expressed as the triple: (Delegation\footnote{\say{D} stands for (standard) delegation while \say{Sp.D} stands for special category delegation. }, Adoption Threshold $\tau_A$, Safety Lag $T_s$). We see for different types of deployment complexities (High/Medium/Low) and approved and agreed priorities (High/Medium/Low), the proposed update policy that will achieve our activation goal. 
