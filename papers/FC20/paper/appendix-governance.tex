...
We require that for each voting period and for every software update submitted, there is always at least a stake percent of $h_a$ honest parties, actively participating in the voting process (i.e., submitting a vote with a value of for/against/abstain).
...
With respect to the voting threshold $\tau_V$, we have seen that for a software update (either a SIP, or an UP) to get approved the following condition should hold: $stake\ in\ favour > \tau_V$. Our voting mechanism, from a security perspective, has essentially two main goals: a) a software update that is not approved by the honest stake majority $h_a/2$, will never be allowed to pass to the next phase in the SU lifecycle and b) a software update that it is approved by the honest stake majority $h_a/2$, will always be allowed to pass to the next phase in the SU lifecycle.

We know that we have at least a percent of $h_a$ honest stake actively voting. Let's assume that we impose a threshold $\tau_V > h_a$.
%Let's assume that we imposed another threshold: $stake\ in\ favour \geq \tau_V + \delta$, where $\delta > 0$. 
 Then, since we need more that $h_a$ votes for the SU to get approved, then the adversary could block the approval, either by voting 'against', or 'abstain', or by not voting at all. So when the voting threshold exceeds the honest stake percentage participating in the voting process, we risk a \emph{Denial of a Majority Result attack}, even if the honest stake $h_a$ votes unanimously.
 %, where we are constantly entering a revoting period, until the software update becomes expired.
 
On the opposite side, if we impose a threshold $\tau_V \leq h_a$,  
...
Let's omit the naive assumption. Clearly, in order for the adversary stake not to have the power to vote for a malicious proposal on its own, we need $\tau_V \geq r \times 100 + \delta$, where $0 < \delta < (1-r) \times 100$. According to our security goals, we want to give to the honest stake majority $h_a/2$ the power to control the result of the voting process. Therefore $\delta$ should be lower bounded by $h_a/2$, which based on Equation \ref{eq3} is also lower bounded by $(1-r) \times 100 / 2$. Which results to the condition: 
\begin{equation} \label{eq4}
\tau_V \geq T + h_a/2  \Leftrightarrow \tau_V \geq 50 \times (r + 1)
\end{equation}
...
Furthermore, we have seen that if the voting threshold is too high then we risk a denial of a majority result attack. In particular, if it exceeds the active honest stake $\tau_V > h_a$, then even if the whole honest stake unanimously votes for a SU, then the adversary still has the power to launch a denial of a majority attack. Therefore, from Equation \ref{eq4}, we choose to take the lower bound for $\tau_V$, so as to keep the voting threshold as low as possible, and hence assume that $\tau_V = T + h_a/2$ and therefore we impose the condition of $T + h_a/2 < h_a$, in order not to risk a denial of a majority attack in the case of a \emph{unanimous result} from the honest stake $h_a$. This, due to Equation \ref{eq1} translates to the following condition:
\begin{equation} \label{eq5}
 h_a > 2/3 \times 100 \Leftrightarrow T < 1/3 \times 100
\end{equation}
Equation \ref{eq5} says that if the active honest stake participating in voting process is more than $66.6\%$, then a unanimous decision from the honest stake cannot be blocked, if the voting threshold is restricted according to Equation \ref{eq4}.  

In the case where we don't have a unanimous decision from the active honest stake $h_a$, then there is still the risk of a denial of a majority attack. Indeed, according to Equation \ref{eq4}, the worst case where the adversary $T$ has full blocking power is when we have a marginal approval of a SU from the honest stake (i.e., a stake in favor result close to $h_a/2$ - so only half of the honest stake supports a proposal) and therefore in that case, we need the whole adversary stake to vote positively, in order to reach the voting threshold. To mitigate this risk, as we have already described above, we propose that a no-majority result leads to a revoting process for the specific SU. This revoting can take place an $rv_{no-majority}$ number of times and only after $rv_{no-majority}$ times of a no-majority result the SU will become expired. Therefore, we are limiting the power of the adversary to block a SU just for a specific voting period and give the chance to the honest stake to reconsider its decision and maybe choose to support a candidate SU with a stronger vote.
