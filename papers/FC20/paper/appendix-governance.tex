...
\item \emph{Approved}. When the $stake\ in\ favour \geq \tau_V$

\item \emph{Rejected}. When the $stake\ against \geq \tau_V$

\item \emph{No-Quorum}. When the $abstaining\ stake \geq \tau_V$. In this case, we revote (i.e., enter one more voting period) for the specific software update. This revoting can take place up to $rv_{no-quorum}$ times, which is a protocol parameter. After that, the software update becomes \emph{expired}.
...
With respect to the voting threshold $\tau_V$, we have seen that for a software update (either a SIP, or an UP) to get approved the following condition should hold: $stake\ in\ favour \geq \tau_V$. Our voting mechanism, from a security perspective, has essentially two main goals: a) a software update that is not approved by the honest stake majority $h_a/2$, will never be allowed to pass to the next phase in the SU lifecycle and b) a software update that it is approved by the honest stake majority $h_a/2$, will always be allowed to pass to the next phase in the SU lifecycle.
...
%Let's assume that we imposed another threshold: $stake\ in\ favour \geq \tau_V + \delta$, where $\delta > 0$. 
 Then, since we need more that $h_a$ votes for the SU to get approved, then the adversaries could block the approval, either by voting 'against', or 'abstain', or by not voting at all. So when the voting threshold exceeds the honest stake percentage, we risk a \emph{Denial of Majority Result attack}, where we are constantly entering a revoting period, until the software update becomes expired.
...
, then the adversaries could potentially approve a malicious SU, if the honest stake $h_a$ does not vote a unanimous rejection (i.e., some part of the honest stake votes \say{against}, some other part part votes \say{abstain} and maybe some part (mistakenly) votes positively) and the adversary stake votes positively, so that in total the voting threshold is reached. 

So, when the voting threshold is too low, we risk the approval of a malicious SU by the adversary stake, while when it is too high we risk the denial of majority result attack. This is a similar situation with the one described in Section \ref{se:informal}.
