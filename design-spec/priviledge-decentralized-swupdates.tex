...
\title{
  Decentralized software updates for stake-based ledgers: Design specification 
...
blockchain.
% TODO: why achieving a decentralized blockchain is important?
...
% TODO: do we say anything about the not having a centrally owned code
...
% TODO: OPTIONAL: explain why not vote and endorse an implementation at the 
%same time?
...
Since the consensus protocol is open source, there might exist multiple 
implementations of the client (in different languages and/or different 
licensing schemes). As we have seen, our mechanism has a separate on-chain 
voting phase during the ideation phase (section \ref{sec:ideation}). In this 
phase a formal-spec/design of the protocol update is submitted (a SIP - System 
Improvement Proposal). So we are \emph{clearly distinguishing the protocol 
update decision from the implementation approval decision}. The latter takes 
place in the second on-chain voting phase, the approval phase (section 
\ref{sec:approval}), which references the approved SIP. So in a 
multi-implementation setting, what you really need to approve and endorse is 
the protocol upgrade and not the implementation per se. The implementation 
adopted by each party is something that we cannot essentially control. However, 
in the approval phase mentioned above, we can still have a ``blessed'' 
\emph{reference implementation} of the upgraded protocol, or even  have a 
single proposal that includes multiple implementations (e.g., in different 
languages) that are approved/blessed by the community.
...
the architecture of our prototype integration with the Cardano node.
