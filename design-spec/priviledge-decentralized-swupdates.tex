...
% TODO: should we mention here (or somewhere) how governance and the lifecycle
% of an update are related? We also need to define what an update is.
...
\item[Approval] In this phase, the information about the implementation of an
  ...
  is referred to as UP. If the implementation deals with a protocol update, then
  the update proposal must contain enough information for the protocol to
  activate it. As in the ideation phase, the stakeholders vote on the
  implementation.
...
would only have to check what proposals are submitted by keys in the submitters
set.
%
...
% We don't claim this solution is complete, but is intended to constitute a
% solid base for adding more features that address the open problems described
% in Section ...
