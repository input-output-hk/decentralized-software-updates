...
\item Determine priorities: a lower version (higher priority) is activated
  before a higher version (lower priority) one; specifying the version gives us
...
\item picking the proposal with the greatest hash, if they have the same stake
  in favor\footnote{Note that this is an even more unlikely scenario, and if the
    experts decide to approve two proposals with the same version at the same
    time, then this might point to a serious problem on the human side.}.
\end{enumerate}
...
Protocol updates (signaled and otherwise) get activated \emph{at the beginning
  of an epoch}. The reason for this is that the ledger and consensus rules rely
...
activation phase.
%
Upon approval these kind of updates the update system simply records this event,
...
It the update concerns an application that uses the blockchain (e.g. a wallet),
...
Just like the other kind of updates, non-protocol updates must have a
...
\item The proposal is overridden by another proposal with the same version that
  ...
  proposals approved with the same version as a proposal approved in a previous
  slot.
...
\item If the candidate proposal does not have enough endorsements, then it is
  ...
  instance due to lack of enough endorsements), we leave the old proposal in the
  queue.
...
\item unlike the Cardano network, the stability window was set to 36 minutes
...
\emph{average end-to-end time} from the transaction submission until it appears in the
ledger. To this end, we have used the same setup as in the throughput
experiments. In other words, a 10 node geographically distributed network,
where $50$ threads per node
constantly submit transactions and we utilize a varying number of voters that
try to vote in a $30$ minute voting period window.
...
Cardano, in a separate branch. The PRIViLEDGE deliverable~\cite{priviledge_d43}
contains a detailed description of the architecture of our prototype integration
...
adopt a decentralized governance scheme. It these systems, the priorities as
...
Torsos~\cite{tezos-white-paper} implements a decentralized update protocol similar
...
\item Quorum: the threshold for approving a proposal uses a notion of quorum.
  ...
  participation.
\end{itemize}
Unlike Tezos' update protocol, the update protocol described in this document
features:
\begin{itemize}
...
A similar approach is proposed by Bingsheng et al. \cite{treasury}, where a
complete treasury system is proposed for blockchain systems, in which liquid
democracy / delegative voting is followed. We also follow the approach of voting
delegation, when technical expertise is required in order to reach a decision
for an update proposal. Similarly, Bingsheng et al. work is focused only on the
treasury system, which covers only the initial phase in the lifecycle of an
update proposal.
